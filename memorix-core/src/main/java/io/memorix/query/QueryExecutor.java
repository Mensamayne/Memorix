...
public class QueryExecutor {
    ...
    public QueryResult executeQuery(String userId, float[] queryVector, QueryLimit limit) {
        return executeQuery(userId, queryVector, limit, new HashMap<>());
    }
    
    /**
     * Execute semantic search query with metadata filtering.
     * 
     * @param userId User ID
     * @param queryVector Query embedding vector
     * @param limit Query limits
    ...
    public QueryResult executeQuery(String userId, float[] queryVector, QueryLimit limit,
        ...
        log.debug("Executing query for user={}, fetchLimit={}, minSimilarity={}, filters={}", 
            userId, fetchLimit, limit.getMinSimilarity(), metadataFilters);
        
        List<MemoryWithSimilarity> candidates = fetchCandidates(userId, queryVector, fetchLimit, metadataFilters);
    ...
    /**
     * Fetch candidate memories from database with metadata filtering.
     */
    private List<MemoryWithSimilarity> fetchCandidates(String userId, 
        ...
        try {
            ...
            // Add metadata filters (JSONB queries)
            ...
            if (metadataFilters != null && !metadataFilters.isEmpty()) {
                for (Map.Entry<String, Object> entry : metadataFilters.entrySet()) {
                    // Use ->> operator to extract as text (already returns text, no casting needed)
                    sql.append(" AND m.metadata->>").append("'").append(entry.getKey()).append("' = ?");
                    params.add(String.valueOf(entry.getValue()));
                }
            }
            
            sql.append(" ORDER BY m.embedding <=> ?::vector ");
            sql.append(" LIMIT ?");
            ...
            log.debug("Params (non-vector): userId={}, filters={}, limit={}", userId, metadataFilters, fetchLimit);
            ...
            throw new StorageException(ErrorCode.QUERY_FAILED,
                ...
                .withContext("metadataFilters", metadataFilters);
        }
    }
    ...
    private List<Memory> applyLimits(List<MemoryWithSimilarity> candidates, QueryLimit limit) {
        ...
        for (MemoryWithSimilarity candidate : candidates) {
            ...
            // Store similarity in metadata for later use in buildMetadata()
            memory.getMetadata().put("similarity", similarity);
            ...
            // Add memory with similarity in metadata
            memory.addMetadata("similarity", similarity);
            results.add(memory);
            currentCount++;
    ...
    private QueryResult.QueryMetadata buildMetadata(int totalFound, 
        ...
        // Calculate average similarity from metadata
        double avgSimilarity = 0.0;
        if (!results.isEmpty()) {
            avgSimilarity = results.stream()
                .filter(m -> m.getMetadata().containsKey("similarity"))
                .mapToDouble(m -> (Double) m.getMetadata().get("similarity"))
                .average()
                .orElse(0.0);
        }
        ...
        public MemoryWithSimilarity mapRow(ResultSet rs, int rowNum) throws SQLException {
            ...
            try {
                ...
                if (metadataJson != null && !metadataJson.isEmpty()) {
                    memory.setMetadata(parseMetadata(metadataJson));
                }
            } catch (SQLException e) {
                // metadata column might not exist in older schemas
